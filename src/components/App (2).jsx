...
function DynamicImageGallery({ project }) {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      if (isMounted) setAspects(ratios);
    });
  ...
  const gridTemplate =
    aspects.length === imageSources.length && imageSources.length > 1
      ? aspects.map((r) => `${r}fr`).join(" ")
      : undefined;
  ...
  return (
    <div className="mb-8">
      <div
        ...
        style={{
          gridTemplateColumns: gridTemplate || (imageSources.length > 1 ? `repeat(${imageSources.length}, 1fr)` : "1fr"),
        }}
      >
        {imageSources.map((src, index) => (
          <div
            key={index}
            className="w-full h-full flex items-center justify-center overflow-hidden rounded-xl bg-neutral-100 dark:bg-neutral-900"
          >
            <ImageSlot
              src={src}
...
const EDUCATION = [
  ...
  },
  {
    ...
    detail: "Completed secondary school in North-West, Itsoseng, with a focus on mathematics and physical sciences.",
  },
];
...
const VALUES = [
...
},
  {
    title: "Growth",
    detail:
      "I believe growth comes through experience, reflection and continuous learning. Every project is an opportunity to become a better engineer.",
  },
  {
    title: "Collaboration",
    detail:
      "I believe the best work happens in positive environments where people support one another, communicate openly and work towards a shared goal.",
  },
  {
    title: "Accountability",
    detail:
      "I believe in taking ownership of my work, being dependable and following through on commitments. Good engineering is built on consistency, responsibility and trust.",
  },
];
    ...
    <div className="max-w-6xl w-full mx-auto my-auto px-5 md:px-8 relative" style={{ zIndex: 2 }}>
      ...
      <Reveal delay={240}>
        <div className="mt-9 flex flex-wrap gap-3">
          ...
          <button
            onClick={() => onNavigate("road-ahead")}
            className="cep-btn-outline cep-focus px-6 py-3.5 text-sm font-medium inline-flex items-center gap-2"
          >
            <Compass size={15} className="cep-btn-icon" /> The Road Ahead
          </button>
        </div>
      </Reveal>
...
const About = () => (
  ...
  <div className="mt-8">
    ...
    <p style={{ color: "var(--ink-soft)" }}>
      My goal is to become a technically excellent Professional Engineer by
      contributing to increasingly complex infrastructure projects and
      continuing to learn from experienced engineers throughout my career.
    </p>
  </div>
...
const ROAD_MILESTONES = [
  {
    ...
    dateLabel: "2020",
    title: "Matriculation",
    detail: "Completed secondary school in Itsoseng, North-West, with a focus on mathematics and physical sciences.",
    icon: GraduationCap,
  ...
  },
  {
    ...
    detail: "Began undergraduate study in Civil Engineering at the University of the Witwatersrand.",
    icon: GraduationCap,
    image: "/images/milestones/wits-start.jpg",
  },
  {
    id: "wits-grad",
    dateLabel: "2025",
    title: "Graduated BSc Eng (Civil), Wits",
    detail: "Graduated after academic projects spanning geotechnical, structural, concrete and transportation engineering.",
    icon: Award,
    image: "/images/milestones/wits-grad.jpg",
  },
  {
    id: "oar-practice",
    dateLabel: "2026",
    title: "ECSA Candidate Engineer — OAR Consultants",
    detail: "Registered as an ECSA Candidate Engineer and practising in transport planning, TIAs and GIS-based modelling at OAR Consultants.",
    icon: Briefcase,
  ...
  },
  {
    ...
    detail: "The next formal milestone in practice — full professional registration.",
    icon: Award,
  ...
  },
  {
    id: "major-infra",
    dateLabel: "Ahead",
    title: "Major Infrastructure Leadership",
    detail: "Leading delivery on infrastructure work at a larger scale and scope.",
    icon: Building2,
  ...
  },
  {
    ...
    detail: "Supporting the next generation of graduate engineers entering practice.",
    icon: HardHat,
  ...
  },
  {
    ...
    detail: "The trajectory continues — future milestones not yet defined.",
    icon: Compass,
    image: null,
    future: true,
    horizon: true,
...
const ROAD_N = ROAD_MILESTONES.length;
const ROAD_CURRENT_INDEX = ROAD_MILESTONES.findIndex((m) => m.current);

/* Catmull-Rom → cubic-bezier: turns a list of (x,y) points into one
   smooth continuous curve (as opposed to straight segments meeting at
   angles at each node) — this is what makes the line read as a single
   flexible string rather than a connect-the-dots polyline. */
...
/* ---------------------------------------------------------------- */
/* useStringWave — a tiny hand-rolled physics simulation (semi-       */
/* implicit Euler spring, one per checkpoint), stepped on every       */
/* animation frame. Low stiffness + near-critical damping = a slow,   */
/* heavy, non-bouncy glide toward its target and back — real motion,  */
/* not a CSS transition snapping between two states. Every checkpoint */
/* close to the hovered one is given a partial target too, so lifting */
/* one visibly tugs its neighbours — the "plucked string" effect —    */
/* and the connecting SVG path is rebuilt from those same live values */
/* every frame, so the line and the dots can never visually separate. */
/* ---------------------------------------------------------------- */
const WAVE_FALLOFF = [1, 0.46, 0.18, 0.05];
const STIFFNESS = 46;
const DAMPING = 15;

function useStringWave(activeIndex) {
  const springs = useRef(ROAD_MILESTONES.map(() => ({ value: 0, velocity: 0 })));
  const nodeEls = useRef([]);
  const pathPastRef = useRef(null);
  const pathFutureRef = useRef(null);
  const rafRef = useRef(null);

  const setNodeRef = (i) => (el) => { nodeEls.current[i] = el; };
  ...
  useEffect(() => {
    ...
    const paint = () => {
      ...
      if (pathPastRef.current) pathPastRef.current.setAttribute("d", smoothPath(pts));
      if (pathFutureRef.current) pathFutureRef.current.setAttribute("d", smoothPath(pts));

      springs.current.forEach((s, i) => {
        const el = nodeEls.current[i];
        if (!el) return;
        const liftPx = s.value * MAX_LIFT;
        const focus = s.value; // 0 = at rest / unfocused, 1 = fully engaged
        el.style.transform = `translateY(${(-liftPx).toFixed(2)}px) scale(${(1 + focus * 0.32).toFixed(3)})`;
        el.style.filter = `blur(${(1 - focus) * 1.1}px)`;
      });
    };
...
const roadFrameVariants = {
  hidden: { opacity: 0 },
  visible: { opacity: 1, transition: { duration: 0.2 } },
  exit: { opacity: 0, transition: { duration: 0.3, ease: ROAD_EASE } },
};

const roadWipeVariants = {
  hidden: { clipPath: "inset(0 100% 0 0)" },
  visible: { clipPath: "inset(0 0% 0 0)", transition: { duration: 1.15, ease: ROAD_EASE, delay: 0.1 } },
};

const roadScanVariants = {
  hidden: { left: "0%", opacity: 0 },
  visible: { left: "100%", opacity: [0, 1, 1, 0], transition: { duration: 1.15, ease: ROAD_EASE, delay: 0.1 } },
};

const roadCaptionVariants = {
  hidden: { opacity: 0, y: 10 },
  visible: { opacity: 1, y: 0, transition: { duration: 0.7, ease: ROAD_EASE, delay: 0.15 } },
  exit: { opacity: 0, y: 6, transition: { duration: 0.4, ease: ROAD_EASE } },
};

const RoadAhead = () => {
  const [activeIndex, setActiveIndex] = useState(null);
  const { setNodeRef, pathPastRef, pathFutureRef } = useStringWave(activeIndex);
  const active = activeIndex == null ? null : ROAD_MILESTONES[activeIndex];

  return (
    <section id="road-ahead" className="py-28 md:py-40 relative overflow-hidden">
      <style>{`
        .cep-road-bracket{ position:absolute; width:16px; height:16px; opacity:0.9; }
      `}</style>
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-8">
        ...
        {/* stage — image, then caption below it, stacked so they can never overlap */}
        <div className="mt-16 md:mt-20 flex flex-col items-center justify-end" style={{ minHeight: 460 }}>
          <div className="flex-1 flex items-end justify-center w-full" style={{ minHeight: 360 }}>
            <AnimatePresence mode="wait">
              {active && active.image && (
                <motion.div
                  key={active.id}
                  variants={roadFrameVariants}
                  initial="hidden"
                  animate="visible"
                  exit="exit"
                ...
                >
                  <div
                    ...
                    style={{ borderRadius: 4, boxShadow: "0 30px 60px -25px rgba(10,11,8,0.35)" }}
                  >
                    {/* base layer: the same photo, held as a flat blueprint-tinted
                        record underneath — this is what's "on file" before it
                        resolves. It's already there; nothing pops from nothing. */}
                    <div className="absolute inset-0">
                      <div style={{ filter: "grayscale(1) contrast(1.15) brightness(0.78)", width: "100%", height: "100%" }}>
                        <ImageSlot src={active.image} alt="" placeholder="Photo placeholder" className="w-full h-full" />
                      </div>
                      <div className="absolute inset-0" style={{ background: "var(--accent)", opacity: 0.24, mixBlendMode: "color" }} />
                    </div>

                    {/* top layer: the true photograph, developing in left to right —
                        a plotter/scan pass, not a spotlight. This is the actual
                        reveal; everything else here is just supporting it. */}
                    <motion.div variants={roadWipeVariants} className="absolute inset-0">
                      <ImageSlot src={active.image} alt={active.title} placeholder="Photo placeholder" className="w-full h-full" />
                    </motion.div>

                    {/* the scan edge itself, tracing the exact boundary of the wipe */}
                    <motion.div
                      variants={roadScanVariants}
                      className="absolute top-0 bottom-0 pointer-events-none"
                      style={{ width: 2, marginLeft: -1, background: "var(--accent)", boxShadow: "0 0 14px 2px var(--accent-soft), 0 0 3px var(--accent)" }}
                    />

                    <div className="pointer-events-none absolute inset-0" style={{ boxShadow: "inset 0 0 0 1px var(--accent-soft)" }} />
                  </div>

                  {/* viewfinder corner brackets */}
                  <span className="cep-road-bracket" style={{ top: -8, left: -8, borderTop: "1.5px solid var(--accent)", borderLeft: "1.5px solid var(--accent)" }} />
                  <span className="cep-road-bracket" style={{ top: -8, right: -8, borderTop: "1.5px solid var(--accent)", borderRight: "1.5px solid var(--accent)" }} />
                  <span className="cep-road-bracket" style={{ bottom: -8, left: -8, borderBottom: "1.5px solid var(--accent)", borderLeft: "1.5px solid var(--accent)" }} />
                  <span className="cep-road-bracket" style={{ bottom: -8, right: -8, borderBottom: "1.5px solid var(--accent)", borderRight: "1.5px solid var(--accent)" }} />

                  <div className="absolute -top-6 left-0 font-mono text-[10px] tracking-[0.14em] uppercase" style={{ color: "var(--accent-ink)" }}>
                    Archive // {active.dateLabel}
                  </div>
                  <div className="absolute -bottom-6 right-0 font-mono text-[10px] tracking-[0.14em] uppercase" style={{ color: "var(--ink-faint)" }}>
                    {String(activeIndex + 1).padStart(2, "0")} / {String(ROAD_N).padStart(2, "0")}
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <div className="mt-10 h-16 flex items-start justify-center text-center px-4">
            <AnimatePresence mode="wait">
              {active && (
                <motion.div
                  ...
                  style={{ maxWidth: 380 }}
                >
                  <div className="font-mono text-[10px] tracking-[0.14em] uppercase" style={{ color: "var(--ink-faint)" }}>
                    {active.dateLabel}
                  </div>
                  <div className="mt-1 text-sm font-medium leading-snug" style={{ color: "var(--ink)" }}>
                    {active.title}
                  </div>
                  <div className="mt-1 text-xs leading-relaxed" style={{ color: "var(--ink-soft)" }}>
                    {active.detail}
                  </div>
                </motion.div>
              )}
        ...
        {/* the line — standalone, no card, no background. Just the string and its checkpoints. */}
        <div
        ...
        >
          ...
          >
            <defs>
              <clipPath id="road-clip-past" clipPathUnits="userSpaceOnUse">
                <rect x="0" y="0" width={(ROAD_CURRENT_INDEX / (ROAD_N - 1)) * 1000} height="120" />
              </clipPath>
              <clipPath id="road-clip-future" clipPathUnits="userSpaceOnUse">
                <rect
                  x={(ROAD_CURRENT_INDEX / (ROAD_N - 1)) * 1000}
                  y="0"
                  width={1000 - (ROAD_CURRENT_INDEX / (ROAD_N - 1)) * 1000}
                  height="120"
                />
              </clipPath>
            </defs>
          ...
          <div className="relative h-full">
            {ROAD_MILESTONES.map((m, i) => {
              ...
              const stepsPastNow = ROAD_CURRENT_INDEX >= 0 ? Math.max(0, i - ROAD_CURRENT_INDEX) : 0;
              const mist = m.future ? Math.max(0.3, 1 - stepsPastNow * 0.16) : 1;
              ...
              return (
                ...
                >
                  <div ref={setNodeRef(i)} style={{ willChange: "transform, filter" }} className="flex flex-col items-center">
                    <span
                  ...
                  <span
                    className="mt-3 font-mono text-[9px] md:text-[10px] tracking-widest uppercase whitespace-nowrap"
                    style={{ color: "var(--ink-faint)" }}
                  >
                    {m.dateLabel}
                  </span>
