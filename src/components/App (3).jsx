...
const EXPERIENCE = [
  ...
  },
  {
    ...
    tag: "Research",
    image: "/images/experience/geotech-lab.jpg",
  },
  {
    id: "vacation-work",
    year: "Jul - Oct 2025",
  ...
  },
  {
    ...
    tag: "Teaching",
    image: "/images/experience/assistant-teacher.jpg",
  },
];
...
/* Experience                                                           */
/* ------------------------------------------------------------------ */
const EXPERIENCE_ROTATIONS = [-5, 3, -4, 5, -2];
    ...
    <section id="experience" className="py-28 md:py-40">
      <div className="max-w-6xl mx-auto px-5 md:px-8">
        ...
        <div className="mt-12 flex flex-col md:flex-row gap-10 md:gap-16 items-start">
          {/* left column stays where it always was — only its width changes,
              to leave the case file room on the right */}
          <div className="max-w-md w-full" onMouseLeave={() => setHoveredIndex(null)}>
            {EXPERIENCE.map((e, i) => (
              <Reveal key={e.id} delay={i * 60}>
                <div
                  ...
                  className="cep-focus flex gap-6 pb-10 relative rounded-lg -mx-2 px-2 transition-colors duration-300 outline-none"
                  style={{ background: hoveredIndex === i ? "var(--pill-bg)" : "transparent" }}
                >
          ...
          {/* the case file — a small stack of records sitting in the space
              that would otherwise just be empty. Hovering (or focusing) an
              entry on the left calmly brings its record to the front; the
              rest settle back, no one of them ever really "gone". */}
          <div
            className="hidden md:block relative flex-shrink-0"
            style={{ width: 260, height: 340, position: "sticky", top: 140 }}
          >
            {EXPERIENCE.map((e, i) => {
              const isActive = hoveredIndex === i;
              const rest = EXPERIENCE_ROTATIONS[i % EXPERIENCE_ROTATIONS.length];
              return (
                <motion.div
                  key={e.id}
                  ...
                  initial={false}
                  animate={{
                    rotate: isActive ? 0 : rest,
                    x: isActive ? "-50%" : `calc(-50% + ${i * 5}px)`,
                    y: isActive ? -14 : i * 9,
                    scale: isActive ? 1.06 : 0.94,
                    opacity: isActive || hoveredIndex == null ? 1 : 0.55,
                  }}
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 4,
                      boxShadow: isActive
                        ? "0 24px 40px -16px rgba(10,11,8,0.28)"
                        : "0 10px 20px -12px rgba(10,11,8,0.18)",
                  ...
                  >
                    ...
                    <div className="mt-2 font-mono text-[9px] tracking-widest uppercase" style={{ color: "var(--ink-faint)" }}>
                      {e.tag} · {e.year}
                    </div>
                  </div>
                </motion.div>
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-8 w-full flex-1 flex flex-col min-h-0">
        ...
        {/* caption zone — text with its picture beside it, centred in whatever
            space is left. Compact by design: the picture is a small companion
            to the text now, not the main event, so this needs far less room
            than before. */}
              ...
              >
                {active.image && (
                  <div
                    ...
                    style={{ width: 104, height: 104, borderRadius: 6, boxShadow: "var(--shadow-card)" }}
                  >
        ...
        {/* the line — standalone, floating gently, pushed up off the very
            bottom edge rather than pinned flush to it */}
        <div className="pb-16 md:pb-24 flex-shrink-0">
          <div
            className="cep-road-float relative"
            style={{ height: 120 }}
