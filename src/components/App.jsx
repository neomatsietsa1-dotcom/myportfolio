...
/* defined yet."                                                       */
/*                                                                     */
/* An evenly-spaced horizontal timeline, anchored toward the bottom of */
/* a tall, quiet section. At rest it shows only a thin floating bar    */
/* and small date marks - nothing more. Hovering (or focusing/tapping) */
/* a checkpoint lifts it, brings up its description directly above     */
/* that checkpoint, and fades in a single supporting photo in the      */
/* empty space above the bar. Nothing moves until you engage with it.  */
/* ------------------------------------------------------------------ */
...
const ROAD_EASE = "cubic-bezier(0.22, 1, 0.36, 1)";

const RoadAhead = () => {
  const [activeId, setActiveId] = useState(null);
  const [stagePos, setStagePos] = useState(null); // px offset of active node, for the tracking caption
  const trackRef = useRef(null);
  const nodeRefs = useRef({});

  const active = ROAD_MILESTONES.find((m) => m.id === activeId) || null;

  const recomputeStagePos = useCallback((id) => {
    const trackEl = trackRef.current;
    const nodeEl = nodeRefs.current[id];
    if (!trackEl || !nodeEl) return;
    const trackRect = trackEl.getBoundingClientRect();
    const nodeRect = nodeEl.getBoundingClientRect();
    const centre = nodeRect.left - trackRect.left + nodeRect.width / 2;
    setStagePos(Math.min(Math.max(centre, 70), trackRect.width - 70));
  }, []);

  const focusMilestone = useCallback(
    (id) => {
      setActiveId(id);
      recomputeStagePos(id);
    },
    [recomputeStagePos]
  );
  ...
  useEffect(() => {
    if (!activeId) return;
    const onResize = () => recomputeStagePos(activeId);
    window.addEventListener("resize", onResize);
    return () => window.removeEventListener("resize", onResize);
  }, [activeId, recomputeStagePos]);
  ...
  return (
    <section id="road-ahead" className="py-28 md:py-40 relative overflow-hidden">
      <style>{`
        @keyframes cep-road-fade-up{
          from{ opacity:0; transform:translateY(8px); }
          to{ opacity:1; transform:translateY(0); }
        }
        @keyframes cep-road-fade{
          from{ opacity:0; transform:scale(0.99); }
          to{ opacity:1; transform:scale(1); }
        }
        .cep-road-caption{ animation: cep-road-fade-up 380ms ${ROAD_EASE}; }
        .cep-road-photo{ animation: cep-road-fade 420ms ${ROAD_EASE}; }
        .cep-road-riser{
          transition: transform 320ms ${ROAD_EASE};
        }
        .cep-road-dot{
          transition: transform 320ms ${ROAD_EASE}, box-shadow 320ms ${ROAD_EASE}, background 320ms ${ROAD_EASE}, border-color 320ms ${ROAD_EASE};
        }
        .cep-road-node:hover .cep-road-riser,
        .cep-road-node:focus-visible .cep-road-riser{ transform: translateY(-6px); }
        .cep-road-node:hover .cep-road-dot,
        .cep-road-node:focus-visible .cep-road-dot{
        ...
        @media (prefers-reduced-motion: reduce){
          .cep-road-caption, .cep-road-photo{ animation:none; }
          .cep-road-riser, .cep-road-dot{ transition:none; }
        }
      `}</style>
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-8">
        ...
        {/* Stage + timeline. Fixed-ish height, flex column, timeline pinned
            toward the bottom via margin-top:auto - the empty space above
            it is where the active checkpoint's photo and caption appear. */}
        <div
          className="mt-14 md:mt-16 flex flex-col"
          style={{ minHeight: 420 }}
        >
          {/* stage: photo, centred, only present while a checkpoint is active */}
          <div className="flex-1 flex items-center justify-center px-4" style={{ minHeight: 200 }}>
            {active && active.image && (
              <div key={active.id} className="cep-road-photo w-full max-w-xs">
                <ImageSlot
                  src={active.image}
                  alt={active.title}
          ...
          {/* tracking caption - sits directly above whichever checkpoint is active */}
          <div className="relative h-16 md:h-20" aria-hidden={!active}>
            {active && (
              <div
                key={active.id + "-caption"}
                className="cep-road-caption absolute bottom-2 text-center"
                style={{
                  left: stagePos ?? "50%",
          ...
          <div
            ...
            className="cep-road-bar mt-6 md:mt-8 relative rounded-full px-6 md:px-10 py-7 md:py-8"
            style={{ background: "var(--surface-raised)", border: "1px solid var(--line-soft)" }}
          >
            {/* baseline */}
            <div className="absolute left-8 right-8 md:left-12 md:right-12" style={{ top: "50%", height: 1, background: "var(--line)" }} />
            ...
            <div className="relative flex items-start justify-between">
              {ROAD_MILESTONES.map((m, i) => {
                ...
                const stepsPastNow = currentIndex >= 0 ? Math.max(0, i - currentIndex) : 0;
                const contrast = m.future ? Math.max(0.35, 1 - stepsPastNow * 0.18) : 1;
                ...
                return (
                  <button
                    ...
                    onMouseEnter={() => focusMilestone(m.id)}
                    onFocus={() => focusMilestone(m.id)}
                    onClick={() => focusMilestone(isActive ? null : m.id)}
                    aria-label={`${m.title} - ${m.dateLabel}`}
                    aria-pressed={isActive}
                    className="cep-road-node cep-focus flex flex-col items-center bg-transparent border-0 p-0 cursor-pointer"
                    style={{ opacity: contrast, filter: m.future ? `blur(${stepsPastNow * 0.15}px)` : "none", flex: "1 1 0" }}
                  >
                    <div className="cep-road-riser flex flex-col items-center">
                      <span
                        className="cep-road-dot rounded-full flex items-center justify-center"
                        style={{
                    ...
                    <Icon size={13} className="mt-1.5 hidden md:block" style={{ color: "var(--ink-faint)" }} />
                  </button>
                );
              })}
            </div>
