...
import {
  ...
  Ruler, Layers, Send, CheckCircle2,FlaskConical, Ship
} from "lucide-react";
...
const PROJECTS = [
  ...
  },
  {
    id: "kinshasa-ferry",
    type: "industry",
    image: "/images/projects/kinshasa-ferry-routes.webp",
    image2: "/images/projects/kinshasa-ferry-feasibility.jpg",
    icon: Ship,
    category: "Transport Planning / Strategic Infrastructure",
    title: "Kinshasa Integrated Ferry Network - Congo River, DRC",
    summary:
      "Planning a modern passenger and freight ferry network to improve mobility across the Congo River through new routes, terminals and multimodal transport integration.",
    role: "Contributing Engineer, OAR Consultants",
    duration: "OAR Consultants",
    software: ["QGIS", "Google Earth", "Microsoft Excel"],
    overview:
      "A multidisciplinary project to develop Kinshasa's future ferry network, creating a reliable transport system that connects communities, major activity centres and key transport hubs across the Congo River.",
    problem:
      "Rapid urban growth and increasing congestion have outpaced the city's transport network. The project explored how the Congo River could become a practical transport corridor, reducing pressure on the road network while improving regional connectivity.",
    objectives: [
      "Develop an integrated passenger and freight ferry network",
      "Plan ferry routes and identify suitable terminal locations",
      "Forecast passenger demand and operational requirements",
      "Develop a phased implementation strategy",
    ],
    process:
      "Contributed to technical report writing, GIS mapping, client presentations, financial modelling and implementation planning. Assisted with route planning, terminal site identification and the preparation of project deliverables using QGIS, Google Earth and Excel.",
    decisions:
      "Route alignments, terminal locations and implementation phases were developed by balancing passenger demand, accessibility, operational efficiency and long-term investment priorities.",
    challenges:
      "Designing a completely new transport network required integrating transport planning, infrastructure, operations and financial planning into a practical solution for one of Africa's fastest-growing cities.",
    solutions:
      "Supported the development of ferry routes, terminal concepts, spatial analyses and implementation plans that formed part of the proposed transport network.",
    results:
      "Contributed to the planning of an integrated ferry system connecting seven terminals and supporting future multimodal transport across Kinshasa.",
    lessons:
      "The project showed how successful infrastructure depends on combining sound engineering with strategic planning, stakeholder engagement and long-term thinking.",
  },
  {
    id: "tsf",
    type: "academic",
  ...
  },
  {
    id: "bottled-water",
    type: "academic",
    image: "/images/projects/bottled-water-screenshot.jpg",
    image2: "/images/projects/bottled-water-equipment.jpg",
    icon: FlaskConical,
    category: "Research / Environmental Engineering",
    title: "Microplastic Pollution in Bottled Water (Honours Research Project)",
    summary:
      "An independent research project investigating microplastic contamination in commercially available bottled water brands using laboratory analysis and microscopy.",
    role: "Researcher",
    duration: "University of the Witwatersrand",
    software: ["ImageJ", "Microsoft Excel"],
    overview:
      "An independent research study investigating the occurrence and characteristics of microplastic particles in bottled drinking water. The research combined laboratory testing, microscopy and image analysis to assess contamination levels and particle characteristics.",
    problem:
      "Growing concern over microplastic contamination in drinking water has highlighted the need to better understand the presence, size distribution and morphology of particles found in commercially available bottled water.",
    objectives: [
      "Quantify microplastic contamination in bottled water samples",
      "Characterise particle size, shape and colour",
      "Compare contamination levels across different bottled water brands",
    ],
    process:
      "Designed and conducted the experimental methodology, including sample preparation, vacuum filtration, microscopic analysis and particle identification. Processed and analysed the collected data using ImageJ and Microsoft Excel before interpreting the results.",
    decisions:
      "Laboratory procedures and analytical methods were selected to minimise contamination while providing consistent particle identification and measurement across all samples.",
    challenges:
      "Maintaining contamination control throughout laboratory testing required careful handling of samples and strict experimental procedures to ensure reliable results.",
    solutions:
      "Implemented controlled laboratory practices, repeated observations and image-based particle analysis to improve the reliability and consistency of the findings.",
    results:
      "Successfully quantified and characterised microplastic particles within bottled water samples, producing a comprehensive research report supported by laboratory observations and statistical analysis.",
    lessons:
      "The project strengthened my research methodology, laboratory practice, data analysis and technical writing skills while reinforcing the importance of evidence-based engineering decision-making.",
  },
  {
    id: "school-tia",
    type: "industry",
  ...
  return (
    <section id="projects" className="relative py-28 md:py-40 overflow-hidden">
      <div className="max-w-6xl mx-auto px-5 md:px-8 relative">
        ...
        {/* filter — three plain marks floating with no border or background:
            "All" as text, Briefcase and GraduationCap as icons. Clicking
            one lifts it, scales it up slightly and turns it accent-blue;
            everything else just quietly recedes back to ink-faint. */}
        <Reveal delay={100}>
          <div className="mt-10 flex items-center">
            <div role="group" aria-label="Filter projects by type" className="flex items-center gap-7 md:gap-8">
              {PROJECT_FILTERS.map((f) => {
                ...
                return (
                  <button
                    ...
                    aria-label={f.value === "all" ? "All projects" : `${f.label} projects`}
                    className="cep-focus bg-transparent border-0 p-0 cursor-pointer"
                    style={{
                      color: isActive ? "var(--accent)" : "var(--ink-faint)",
                      transform: isActive ? "translateY(-5px) scale(1.22)" : "translateY(0) scale(1)",
                      filter: isActive ? "drop-shadow(0 10px 14px var(--accent-soft))" : "none",
                      transition: `transform 420ms ${`cubic-bezier(${ROAD_EASE.join(",")})`}, color 420ms ease, filter 420ms ease`,
                    }}
                  >
                    {Icon ? (
                      <Icon size={19} strokeWidth={1.8} />
                    ) : (
                      <span className="font-display text-[15px] font-medium">All</span>
                    )}
                  </button>
                );
              })}
            ...
            <span
              className="ml-10 md:ml-12 font-mono text-sm tabular-nums"
              style={{ color: "var(--ink-faint)" }}
            >
              {filtered.length}
            </span>
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((p, i) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0, transition: { duration: 0.45, ease: ROAD_EASE, delay: i * 0.03 } }}
                exit={{ opacity: 0, y: -6, transition: { duration: 0.25, ease: ROAD_EASE } }}
                transition={{ layout: { type: "tween", duration: 0.45, ease: ROAD_EASE } }}
              >
                <ProjectCard
                  project={p}
                  open={openId === p.id}
    ...
    <div className="max-w-6xl mx-auto px-5 md:px-8">
      ...
      <div className="grid md:grid-cols-3 gap-10 mt-8">
        <Reveal className="md:col-span-1">
          ...
          <a
  href="https://drive.google.com/uc?export=download&id=1KRo8lfTOOt9flpFv5VGsO_HKjkuDBkOn"
  target="_blank"
